...
export function CommandPalette({ open, onOpenChange, spaces, returnFocus }: { open: boolean; onOpenChange: (open: boolean) => void; spaces: SpaceSummary[]; returnFocus: () => void }) {
  ...
  return <Dialog open={open} onOpenChange={onOpenChange}><DialogContent className="palette-enter top-[8dvh] max-h-[84dvh] w-[calc(100%-2rem)] max-w-2xl translate-y-0 gap-0 overflow-y-auto p-0" onOpenAutoFocus={e => { e.preventDefault(); input.current?.focus(); }} onCloseAutoFocus={e => { e.preventDefault(); returnFocus(); }}>
    <div className="border-b border-border px-5 pb-4 pt-5"><DialogTitle className="pr-12 text-base">Buscar documentação</DialogTitle><DialogDescription className="mt-2">Páginas e seções publicadas, em um só lugar.</DialogDescription></div>
    <div className="flex items-center gap-3 border-b border-border px-5"><Search size={20} className="text-primary" /><label htmlFor={`${id}-input`} className="sr-only">Termo de busca</label><input ref={input} id={`${id}-input`} role="combobox" aria-autocomplete="list" aria-expanded={true} aria-controls={`${id}-results`} aria-activedescendant={state === 'ready' && items[selected] ? `${id}-${selected}` : undefined} className="h-16 min-w-0 flex-1 bg-transparent text-base" placeholder="Comando, procedimento ou decisão…" value={query} onChange={e => setQuery(e.target.value)} onKeyDown={e => { if (e.nativeEvent.isComposing || state !== 'ready') return; if (e.key === 'ArrowDown' || e.key === 'ArrowUp') { e.preventDefault(); if (items.length) setSelected((selected + (e.key === 'ArrowDown' ? 1 : -1) + items.length) % items.length); } if (e.key === 'Enter' && items[selected]) { e.preventDefault(); go(items[selected]!); } }} /></div>
    <div className="flex flex-wrap items-center justify-between gap-2 px-5 py-3"><label className="text-sm" htmlFor={`${id}-scope`}>Espaço</label><select id={`${id}-scope`} value={scope} onChange={e => setScope(e.target.value)} className="min-h-11 max-w-full rounded-lg border border-input bg-card px-3 text-sm"><option value="">Todos os espaços</option>{spaces.map(s => <option key={s.id} value={s.slug}>{s.name}</option>)}</select></div>
    <p role="status" className="px-5 pb-3 text-sm text-muted-foreground">{state === 'loading' ? 'Buscando…' : state === 'error' ? 'Não foi possível buscar a documentação.' : `${query ? items.length + ' resultados' : 'Atualizados recentemente'}${more ? ' · refine a busca para ver mais' : ''}`}</p>
    {state === 'error' && <button className="mx-5 mb-4 min-h-11 rounded-lg border border-input px-4" onClick={() => setRetry(v => v + 1)}>Tentar novamente</button>}
    <ul ref={list} role="listbox" aria-label="Resultados" aria-busy={state === 'loading'} id={`${id}-results`} className="max-h-[42dvh] overflow-y-auto px-2 pb-2">{state === 'ready' && items.map((item,i) => <li key={item.pageId} id={`${id}-${i}`} role="option" aria-selected={selected === i} onMouseDown={e => e.preventDefault()} onPointerMove={() => setSelected(i)} onClick={() => go(item)} className={`flex cursor-pointer items-start gap-3 rounded-lg border-l-2 p-3 ${i === selected ? 'border-primary bg-muted' : 'border-transparent'}`}><FileText size={18} className="mt-1 shrink-0 text-primary" /><div className="min-w-0 flex-1"><p className="font-medium">{item.title}</p><p className="mt-1 font-mono text-xs text-muted-foreground">{item.spaceName}{item.sectionTitle ? ` / ${item.sectionTitle}` : ''}</p><p className="mt-2 line-clamp-2 text-sm text-muted-foreground">{item.snippet}</p></div><ArrowUpRight size={16} className="mt-1 shrink-0" /></li>)}</ul>
    {state === 'ready' && !items.length && <p className="px-5 pb-8 text-sm">Nenhuma página encontrada. Tente outro termo ou espaço.</p>}
    <div className="border-t border-border px-5 py-3 font-mono text-xs text-muted-foreground">↑ ↓ selecionar · Enter abrir · Esc fechar</div>
