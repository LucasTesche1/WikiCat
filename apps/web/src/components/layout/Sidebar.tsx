...
import { ChevronDown, ChevronRight, FileText, FolderOpen, Library, Tags, Wrench } from 'lucide-react';
...
function contains(node: PageTreeNode, path: string): boolean {
  return path.endsWith(`/p/${node.id}`) || node.children.some(n => contains(n, path));
}
...
function Branch({ node, slug, depth }: { node: PageTreeNode; slug: string; depth: number }) {
  ...
  const [open, setOpen] = useState(() => {
    try { return sessionStorage.getItem(key) === '1' || contains(node, pathname); } catch { return contains(node, pathname); }
  });
  useEffect(() => { if (contains(node, pathname)) setOpen(true); }, [pathname, node]);
  return <li><div className="flex min-w-0 items-center">
    {node.children.length > 0 ? <button type="button" aria-expanded={open} aria-label={`${open ? 'Collapse' : 'Expand'} ${node.title}`} className="workspace-icon shrink-0"
...
export function Sidebar({ spaces }: { spaces: SpaceSummary[] }) {
  ...
  return <>
    <Link to="/" className="flex min-h-[76px] items-center gap-3 border-b border-border px-5">
      <SystemLogo />
      <span><span className="block text-lg font-semibold tracking-tight">WikiCat</span><span className="font-mono text-xs text-muted-foreground">Tool console</span></span>
    </Link>
    <nav aria-label="Primary navigation" className="flex-1 overflow-y-auto p-3">
      <NavLink end to="/" className="nav-row"><Library size={18} />Overview</NavLink>
      {user?.role === 'admin' && <NavLink to="/admin/templates" className="nav-row"><Wrench size={17} />Templates</NavLink>}
      <div className="eyebrow mb-3 mt-8 px-3">SPACES</div>
      {spaces.map(space => <div key={space.id} className="mb-2"><NavLink end to={`/s/${space.slug}`} className="nav-row"><FolderOpen size={17} className="shrink-0" /><span className="min-w-0 flex-1 truncate">{space.name}</span><span className="font-mono text-xs text-muted-foreground">{space.pageCount}</span></NavLink>
        {slug === space.slug && <div className="mt-1"><PageBranch nodes={tree} slug={space.slug} />{error && <button className="nav-row underline" onClick={() => setRetry(v => v + 1)}>Reload pages</button>}<NavLink to={`/s/${space.slug}/tags`} className="nav-row text-muted-foreground"><Tags size={15} />Tags</NavLink></div>}
      </div>)}
      {!spaces.length && <p className="px-3 text-sm leading-6 text-muted-foreground">Spaces will appear here.</p>}
    </nav>
    <div className="border-t border-border px-5 py-4 font-mono text-xs text-muted-foreground">WIKICAT / INTERNAL TOOL</div>
  </>;
}
